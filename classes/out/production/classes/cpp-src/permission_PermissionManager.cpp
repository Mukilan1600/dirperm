...
JNIEXPORT void JNICALL Java_permission_PermissionManager_setDirectoryPermissions(JNIEnv *env, jclass thisClass, jstring fileNameJ, jstring userNameJ, jobject obj_entry)
{
      ...
      jclass cls_perm_ent = env->GetObjectClass(obj_entry);

      // jclass cls_access_type = env->FindClass("permission/PermissionEntry$AccessType");
      // jfieldID fld_access_type_grant = env->GetStaticFieldID(cls_access_type, "GRANT", "Lpermission/PermissionEntry$AccessType;");
      // jobject obj_acc_grant = env->GetStaticObjectField(cls_access_type, fld_access_type_grant);
      long grnt_acc_mask = MakeAccessMask(env, cls_perm_ent, obj_entry);
      // , deny_acc_mask = MakeAccessMask(env, cls_perm_ent, obj_deny_entry);

      BuildExplicitAccessWithName(&exp_access, (LPSTR)userName, 0, SET_ACCESS, SUB_CONTAINERS_AND_OBJECTS_INHERIT);
      ...
      BuildExplicitAccessWithName(&exp_access, (LPSTR)userName, grnt_acc_mask, DENY_ACCESS, SUB_CONTAINERS_AND_OBJECTS_INHERIT);
